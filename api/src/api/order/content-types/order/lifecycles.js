...
module.exports = {
  async afterCreate(event) {
    ...
    try {
      const orderInfo = await getOrderInfo(result.id);
      if (orderInfo && orderInfo.productDetails) {
        productDetailsHtml = buildProductDetailsHtml(orderInfo.productDetails);
      } else {
        console.error("Invalid response structure from getOrderInfo");
      }
...
async function getOrderInfo(orderId) {
  try {
    const response = await axios.get(
      `http://localhost:1337/api/orders/${orderId}?populate=*`
    );
    return response.data;
  } catch (error) {
    throw new Error(`Error fetching order info: ${error.message}`);
  }
}

function buildProductDetailsHtml(productDetails) {
  if (!productDetails) {
    return "";
  }
  ...
  if (Array.isArray(productDetails)) {
    ...
    return `
      <div>
        <p><strong>Produkt:</strong> ${productDetails.productname}</p>
        <p><strong>Produkt ID:</strong> ${productDetails.productid}</p>
      </div>
    `;
  } else {
    console.error("Unexpected productDetails type:", typeof productDetails);
    return "";
  }
} */
